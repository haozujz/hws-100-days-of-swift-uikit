...
class ImageViewController: UIViewController {
	...
	override func loadView() {
        ...
        // Solution 1) Better animation code in project 15, smarter with weak self closure capture list
        // Solution 2) Destroy (ie. invalidate) timer when no longer needed to break strong reference cycle, in viewWillDisappear()
...
// Initializer UImage(named:) puts image into an image cache for reuse, useful if that image is likely to be reused. UIImage also auto clears itself if it senses memory pressure
